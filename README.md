...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create lightningseo-64d7
  fly apps create lightningseo-64d7-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app lightningseo-64d7
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app lightningseo-64d7-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'lightningseo-64d7-staging' does not match app name in config file 'lightningseo-64d7'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name lightningseo-64d7-db
  fly postgres attach --app lightningseo-64d7 lightningseo-64d7-db

  fly postgres create --name lightningseo-64d7-staging-db
  fly postgres attach --app lightningseo-64d7-staging lightningseo-64d7-staging-db
  ```
